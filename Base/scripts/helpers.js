...
const load_settings = (id, settings) => {
	...
	for (field in settings) {
		...
		if (field === 'is_buy_method_ids' || field === 'is_sell_method_ids') {
			...
			_el = document.querySelector(`#${id} #input_${field} ~ label[data-inner-text-alt], #${id} #${field} ~ label[data-inner-text-alt], #${id} #input_swap_${field} ~ label[data-inner-text-alt], #${id} #is_swap_${field} ~ label[data-inner-text-alt], #${id} #copy_input_${field} ~ label[data-inner-text-alt], #${id} #copy_${field} ~ label[data-inner-text-alt]`);
			if (_el) {
...
const navigate = () => {
	let navigationElement = (() => {
		...
		let jbu = Number(get_url_var('jbu')) || 0,
			...
			token_link = `&chain=${DATA.CHAIN}` + (DATA.pair && DATA.pair !== DATA.ZERO && DATA.pair !== DATA.IMAGINARY_PEG && `&pair=${DATA.pair}` || '') + (DATA.token && DATA.token !== DATA.ZERO && `&token=${DATA.token}` || '');
...
const get_decimals = async (address, chain) => {
	...
	if (!DATA.decimals[address]) {
		try {
			DATA.decimals[address] = await contract(address, undefined, chain, true).decimals().catch(e => DATA.ETHER_BASE);
		} catch (e) { }
	}
...
const get_symbol = async (address, chain) => {
	...
	if (!DATA.symbols[address]) {
		try {
			DATA.symbols[address] = (await contract(address, undefined, chain, true).symbol().catch(e => '')).slice(0, 32).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/[\\'"]/g, '');
		} catch (e) { }
	}
...
const get_nft_image = async (type, nft_id, chain_id) => {
	...
	let json_file = DATA.NFT_CACHE[type][nft_id] || await contract(DATA.CHAINS[chain_id || DATA.CHAIN_ID][DATA.NFT_TYPES_MAP[type]]).tokenURI(_hex(nft_id)).catch(_ => null);
...
const isLQ = async (address, symbol) => {
	if (typeof DATA.is_lq[address] === 'undefined') {
		if (DATA.is_lq[address] = (symbol || await get_symbol(address)).endsWith('-LP')) {
			try {
				DATA.is_lq[address] = await contract(address, undefined, undefined, true).token0().catch(e => DATA.ZERO);
			} catch (e) {
...
const printSubNumbers = (num) => {
	return ' ' + `${num}`.split('').map(v => subNumbers[Number(v)]).join('') + ' ';
};
...
function fallbackCopyTextToClipboard(text, el, old_tooltip_data) {
	...
	try {
		...
		if (old_tooltip_data) {
			setTimeout(() => { el.dataset.tooltip = old_tooltip_data }, 1000);
		} else {
			setTimeout(() => { el.removeAttribute('data-tooltip') }, 1000);
		}
	} catch (err) {
...
const set_main_sell_button_pids = () => {
	elementify('Swap__SellButton').querySelector('[data-action="sell"]').dataset.pids = get_active_positions().map(v => v._id).join(',');
};
...
const handle_scroll = (id, stopper, callback, percent) => {
	...
	const touchstartCallBack = ev => { /* calculating start-touch position */
		tsX = ev.touches[0].clientX;
	 	tsY = ev.touches[0].clientY;
	};

	const touchendCallBack = ev => { /* calculating end-touch position and enable/disable moredata */
	...
	const scrollCallback = ev => {
		...
		if (feed_list_view.scrollTop + feed_list_view.offsetHeight >= feed_list_view.scrollHeight * (percent || 0.6)) { /* 60 percent load by default */
