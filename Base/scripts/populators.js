...
const handle_holder = (data) => {
	if (DATA.token_data && DATA.token_data.i) {
		...
		if (holder) {
			...
			document.querySelector(`[data-holder-percent="${data.D}"]`).innerHTML = `${Big(100).mul((Number(DATA.token_data.i.total_supply) && data.bl) || 0).div(Number(DATA.token_data.i.total_supply) && DATA.token_data.i.total_supply || 1).round(1)}%`;
		} else if (Big(100).mul((Number(DATA.token_data.i.total_supply) && data.bl) || 0).div(Number(DATA.token_data.i.total_supply) && DATA.token_data.i.total_supply || 1).gt(0.1)) {
...
const set_current_position = async (pos, now) => {
	...
	await Promise.all([
		...
		(async () => {
			pos.balance = Big(await contract(pos.token).balanceOf(pos.slot))
				.div(typeof pos.decimals !== 'undefined' && Big(10).pow(pos.decimals) || await get_decimals_power(pos.token));
		})()
	]);
	...
	if (pos.token === DATA.token && DATA.slots[DATA.CHAIN].filter(v => v.address === pos.slot && v.is_active && v.selected).length) {
		...
		if (pos.is_stop_loss && Number(pos.stop_loss_trigger)) {
			term += `<br>${pos.is_stop_loss_trailing && 'TRAILING ' || ''}STOP LOSS [x${formatFiatNumberNoSpaces(pos.stop_loss_trigger)}]: $${formatFiatNumberNoSpaces(Big(pos.stop_loss_trigger).mul(pos.is_stop_loss_trailing && Number(pos.ATH || 1) || 1).mul(pos.enter_price.gt(0) && pos.enter_price || DATA.token_price.div(DATA.WPEG_PRICE)).mul(pos.peg))}`;
		}
		...
		if (Big(pos.balance).eq(0)) {
			pos.balance = Big((DATA.conf.plans || []).filter(v => v[0] === (DATA.view_chain === 'ALL' && DATA.CHAIN || DATA.view_chain) && v[1] === 1).length && 0.997 || 0.99).mul(pos.amount).div(DATA.token_price).div(DATA.ETHER);
		}
	...
	return [term.replace('<br>', ''), `<div class="item">
	...
	<div class="row row-2 mt-12px d-flex">
		<a href="${DATA.EXPLORER}/token/${pos.token}?a=${pos.slot}" target="_blank" rel="nofollow" class="time d-flex btn btn-has-icon" data-tooltip="${pos.created_at.toLocaleDateString()} ${pos.created_at.toLocaleTimeString()}" data-tooltip-right>
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md fs-0 mr-4px">
				<rect fill="#333333" width="16" height="16" rx="100%"/>
		...
		<div class="price d-flex">
			${icon} <div class="text-truncated" data-balance="${pos.balance.mul(DATA.token === pos.token && DATA.token_price || pos.price).div(DATA.WPEG_PRICE)}">$${formatFiat(pos.balance.mul(DATA.token === pos.token && DATA.token_price || pos.price))}</div>
		</div>
	</div>
...
const set_history_position = async (pos, now) => {
	...
	<div class="data">
		<div class="d-flex">
			...
			<a href="${DATA.EXPLORER}/token/${pos.token}?a=${pos.slot}" target="_blank" rel="nofollow" class="text-truncated text-${status}" data-timer="${pos.created_at.getTime()}">${timeDifference(now || Date.now(), pos.created_at.getTime())}</a>
		</div>
		...
		<div class="d-flex mt-12px">
			...
			<div class="text-truncated text-${status}">${formatFiatNumber(pos.sold.gt(0) && pos.sold || pos.bought)} <span data-token="${_t1}" data-action="token" data-pair="${pos.pair}" data-pair-router="${pos.router}">${t1}</span></div>
		</div>
		...
		for (; i < positions.length; ++i) {
			...
			if (!_positions[positions[i].pid]) {
				_positions[positions[i].pid] = {
					...
					bought: Big(positions[i].traded || 0).mul(positions[i].enter_price || 0).mul(positions[i].peg),
					sold: Big(0)
				};
				...
				if (['B', 'b'].includes(positions[i].status)) {
					_positions[positions[i].pid].bought = _positions[positions[i].pid].bought.add(Big(positions[i].traded || 0).mul(positions[i].last_price || 0).mul(positions[i].peg));
				} else if (['S', 's'].includes(positions[i].status)) {
					_positions[positions[i].pid].sold = _positions[positions[i].pid].sold.add(Big(positions[i].traded || 0).mul(positions[i].last_price || 0).mul(positions[i].peg));
				}
			}
...
const get_address_transaction = async (tx) => {
	...
	let icon, title = DATA.selected_slot, i;
...
const get_user_token_balace_in_wpeg = async (tx) => {
	...
	try {
		let price = Big((await contract(tx.R).getAmountsOut(await contract(tx.k).balanceOf(DATA.selected_copy_slot || DATA.selected_slot).catch(_ => 0), tx._b && [...tx.known_tokens_order].reverse() || tx.known_tokens_order).catch(_ => Array.from({ length: tx.known_tokens_order.length }, _ => 0)))[tx.known_tokens_order.length - 1])
			.div(DATA.ETHER);
...
const get_transaction = async (tx) => {
	...
	<div class="action">
		${icon}
		<a href="${DATA.EXPLORER}/tx/${tx.h}" target="_blank" rel="nofollow" class="time ml-6px" data-timer="${tx.c.getTime()}" data-tooltip="${tx.c.toLocaleDateString()} ${tx.c.toLocaleTimeString()}" data-tooltip-right>${timeDifference(now, tx.c.getTime())}</a>
	</div>
...
const parse_copied_txs = async (txs, helper_data) => {
	...
	for (i = 0; i < _transactions_order.length; ++i) {
		...
		_transactions[_transactions_order[i]]._s = DATA.conf.connected && !DATA.copy_settings[DATA.selected_copy_slot][_transactions[_transactions_order[i]].w] || !DATA.copy_settings[DATA.selected_copy_slot][_transactions[_transactions_order[i]].w]._strategy_manual;
		...
		_transactions[_transactions_order[i]].balance = _transactions[_transactions_order[i]].known_tokens[_transactions[_transactions_order[i]].known_tokens_order[_transactions[_transactions_order[i]].known_tokens_order.length - 1]].A;
...
const add_wallets = async () => {
	...
	if (!DATA.copy_settings_ordered[DATA.CHAIN]) {
	...
	for (i = 0; i < DATA.copy_settings_ordered[DATA.CHAIN].length; ++i) {
		...
		tmp = {};
		slot_wallet_balance = (DATA.copy_transactions[DATA.selected_copy_slot] || []).filter(v => v.w === setting.address)
			.reduce((a, n) => {
	...
	<div class="header">
		...
		<div class="name text-white text-truncated">${`${setting.title}`.trim() || DATA.copy_settings_ordered[DATA.CHAIN][i]}</div>
		...
		<div class="actions ml-auto">
			<button class="btn btn-has-icon" data-togglable="ModalDeleteWallet" data-tooltip="Delete" data-tooltip-left data-slot="${setting.slot}" data-wallet="${DATA.copy_settings_ordered[DATA.CHAIN][i]}">${DATA.CANCEL_SVG_BLACK}</button>

			<label class="checkbox-svg ml-6px" data-tooltip="${!setting.is_spectator && 'Pause' || 'Enable'}" data-tooltip-alt="${!setting.is_spectator && 'Enable' || 'Pause'}" data-tooltip-left data-slot="${setting.slot}" data-wallet="${DATA.copy_settings_ordered[DATA.CHAIN][i]}" data-action="toggle_copy_wallet_state">
				...
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md" data-slot="${setting.slot}" data-wallet="${DATA.copy_settings_ordered[DATA.CHAIN][i]}" data-action="toggle_copy_wallet_state">
					...
					<path d="M8 15A7 7 0 118 1 7 7 0 018 15ZM8 16A8 8 0 108 0 8 8 0 008 16ZM6.521 5.055A.5.5 0 017.041 5.093L10.541 7.593A.5.5 0 0110.541 8.407L7.041 10.907A.5.5 0 016.25 10.5V5.5A.5.5 0 016.521 5.055Z" data-slot="${setting.slot}" data-wallet="${DATA.copy_settings_ordered[DATA.CHAIN][i]}" data-action="toggle_copy_wallet_state"></path>
					<path d="M8 15A7 7 0 118 1 7 7 0 018 15ZM8 16A8 8 0 108 0 8 8 0 008 16ZM6.5 5A.5.5 0 017 5.5V10.5A.5.5 0 016 10.5V5.5A.5.5 0 016.5 5ZM9.5 5A.5.5 0 0110 5.5V10.5A.5.5 0 019 10.5V5.5A.5.5 0 019.5 5Z" data-slot="${setting.slot}" data-wallet="${DATA.copy_settings_ordered[DATA.CHAIN][i]}" data-action="toggle_copy_wallet_state"></path>
				</svg>
			</label>

			<button class="btn btn-has-icon ml-6px" data-togglable="ModalCopyPositionSettings" data-tooltip="Settings" data-tooltip-left data-slot="${setting.slot}" data-wallet="${DATA.copy_settings_ordered[DATA.CHAIN][i]}" data-action="load-wallet-settings">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md">
...
const analyze_transactions = async (el, txs, is_full) => {
	let _transactions = {},
		...
		promises = [],
		addresses = DATA.slot_copies_addresses
			&& DATA.slot_copies_addresses.map(v => Object.keys(DATA.copy_settings[v] || (DATA.copy_settings[v] = {}))).flat()
			|| Object.keys(DATA.copy_settings[DATA.selected_copy_slot]),
		i = 0;
	...
	for (i = 0; i < _transactions_order.length; ++i) {
		promises.push((async (_i) => {
			if (!DATA.copy_settings[DATA.selected_copy_slot]) {
				DATA.copy_settings[DATA.selected_copy_slot] = {};
			}

			_transactions[_transactions_order[_i]]._s = DATA.conf.connected && !DATA.copy_settings[DATA.selected_copy_slot][_transactions[_transactions_order[_i]].w] || !DATA.copy_settings[DATA.selected_copy_slot][_transactions[_transactions_order[_i]].w]._strategy_manual;
			...
			_transactions[_transactions_order[_i]].balance = _transactions[_transactions_order[_i]].known_tokens[_transactions[_transactions_order[_i]].known_tokens_order[_transactions[_transactions_order[_i]].known_tokens_order.length - 1]].A;
	...
	el.innerHTML = (await Promise.all(promises)).flat().filter(v => v).join('');
};

const set_wallet_slot_transactions = async () => {
	let container, trials = 150;
	...
	if (container) {
		if (!DATA.copy_transactions || !DATA.copy_transactions[DATA.selected_copy_slot] || !DATA.copy_transactions[DATA.selected_copy_slot].length) {
			container.innerHTML = '<div class="transaction">Spectating...</div>';
		} else {
...
const set_wallet_asset = async (is_origin, token, amounts, chain) => {
	...
	<div class="price">
		<div data-balance="${Big(amounts.bl || 0).mul(amounts.p || 0)}">$${formatFiat(Big(amounts.bl || 0).mul(amounts.p || 0).mul(DATA.WPEG_PRICE))}</div>
	</div>
	<div class="balance">
...
const set_wallet_assets = async (is_origin, title) => {
	...
	if (typeof title === 'undefined') {
		title = ~DATA.wallet_slots_pagination_idx && DATA.slots[DATA.CHAIN][DATA.wallet_slots_pagination_idx]
...
const set_wallet_slot = (slot, i, chain) => {
	...
	<div class="balance text-truncated slot-total-balance-${chain}-${slot.address}" data-balance="${Big(Number(old_balance) && old_balance || slot.total_balance)}">$${formatFiat(Big(Number(old_balance) && old_balance || slot.total_balance).mul(DATA.WPEG_PRICE))}</div>
	...
	<div class="actions">
		<button class="btn btn-has-icon" data-tooltip="Destroy slot" data-tooltip-left data-migrate-slot="${slot.address}" data-action="set-migration-slot" data-migrate-slot="">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md" data-migrate-slot="${slot.address}" data-action="set-migration-slot" data-migrate-slot="">
				...
				<path d="M8 15A7 7 0 118 1 7 7 0 018 15ZM8 16A8 8 0 108 0 8 8 0 008 16ZM4.646 4.646A.5.5 0 015.354 4.646L8 7.293 10.646 4.646A.5.5 0 0111.354 5.354L8.707 8 11.354 10.646A.5.5 0 0110.646 11.354L8 8.707 5.354 11.354A.5.5 0 014.646 10.646L7.293 8 4.646 5.354A.5.5 0 014.646 4.646Z" data-migrate-slot="${slot.address}" data-action="set-migration-slot" data-migrate-slot=""></path>
			</svg>
		</button>

		<label class="checkbox-svg ml-6px" data-tooltip="${slot.is_active ? 'Pause slot' : 'Enable slot'}" data-tooltip-alt="${slot.is_active ? 'Enable slot' : 'Pause slot'}" data-slot="${slot.address}" data-tooltip-left data-slot="${slot.address}" data-wallet="${DATA.ZERO}" data-action="toggle_copy_wallet_state">
			<input type="checkbox" class="slot-${slot.address} visually-hidden" autocomplete="off" ${slot.is_active ? 'checked' : ''}>
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md" data-slot="${slot.address}" data-wallet="${DATA.ZERO}" data-action="toggle_copy_wallet_state">
				...
				<path d="M8 15A7 7 0 118 1 7 7 0 018 15ZM8 16A8 8 0 108 0 8 8 0 008 16ZM6.521 5.055A.5.5 0 017.041 5.093L10.541 7.593A.5.5 0 0110.541 8.407L7.041 10.907A.5.5 0 016.25 10.5V5.5A.5.5 0 016.521 5.055Z" data-slot="${slot.address}" data-wallet="${DATA.ZERO}" data-action="toggle_copy_wallet_state"></path>
				<path d="M8 15A7 7 0 118 1 7 7 0 018 15ZM8 16A8 8 0 108 0 8 8 0 008 16ZM6.5 5A.5.5 0 017 5.5V10.5A.5.5 0 016 10.5V5.5A.5.5 0 016.5 5ZM9.5 5A.5.5 0 0110 5.5V10.5A.5.5 0 019 10.5V5.5A.5.5 0 019.5 5Z" data-slot="${slot.address}" data-wallet="${DATA.ZERO}" data-action="toggle_copy_wallet_state"></path>
			</svg>
		</label>
		...
		<button class="btn btn-has-icon ml-6px" data-tooltip="Send assets" data-tooltip-left data-chain="${chain}" data-slot="${slot.address}">
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md" data-chain="${chain}" data-slot="${slot.address}">
				...
				<path d="M1 8A7 7 0 1015 8 7 7 0 001 8ZM16 8A8 8 0 110 8 8 8 0 0116 8ZM4.5 7.5A.5.5 0 004.5 8.5H10.293L8.146 10.646A.5.5 0 008.854 11.354L11.854 8.354A.5.5 0 0011.854 7.646L8.854 4.646A.5.5 0 108.146 5.354L10.293 7.5H4.5Z" data-chain="${chain}" data-slot="${slot.address}"></path>
			</svg>
		</button>
	</div>
</div>`;
...
const set_wallet_chain_slots = (chain_id, chain, chain_assets, slots, is_first) => {
	let balance = Big(chain === DATA.CHAIN && DATA.conf.vault || 0).add(slots.reduce((a, b) => a.add(b.total_balance), Big(0))),
		_slots = slots.map((v, k) => set_wallet_slot(v, k, chain)).join('') || `<div class="slot btn w-100 text-left"><div class="name text-truncated" data-action="create-slot" data-chain="${chain}" data-pay-chain="${chain_id}">Create Slot</div></div>`;
...
const set_wallet_chains_slots = async () => {
	...
	for (chain_id of DATA.CHAINS_ORDER) {
		html = `${html}${set_wallet_chain_slots(chain_id, DATA.CHAINS[chain_id].CHAIN, DATA.CHAINS[chain_id].CHAIN_ASSETS, DATA.slots[DATA.CHAINS[chain_id].CHAIN] || (DATA.slots[DATA.CHAINS[chain_id].CHAIN] = []), is_first)}`;
				...
				nameChanger.querySelector('button[data-tooltip="Confirm"]').addEventListener('click', event => {
					if (nameChanger.firstElementChild.value.trim()) {
						nameButton.firstElementChild.innerText =
							DATA.slot_name = DATA.slots[DATA.slot_name_chain = nameButton.dataset.chain][nameButton.dataset.slotIdx].title =
	...
	${tokens}
				...
				assetSelector.querySelector('button[data-tooltip="Confirm"]').addEventListener('click', e => {
					...
					let slots = DATA.slots[_chain].map((slot, k) => slot.address !==  _slot && `<div class="slot-selector-item">
...
const set_original_wallet_asset = async (data, i) => {
	...
	<div class="text-truncated" data-pay-slot="SLOT_PLACEHOLDER" data-pay-slot-idx="SLOT_IDX_PLACEHOLDER" data-action="prepare_deposit" data-pay-token="${data.k}">${await get_symbol(data.k)} <span data-balance="${bl}">$${formatFiat(bl.mul(DATA.WPEG_PRICE))}</span></div>
	<label for="LABEL_PLACEHOLDER" class="btn btn-has-icon ml-12px" data-tooltip="Select" data-tooltip-left data-pay-slot="SLOT_PLACEHOLDER" data-pay-slot-idx="SLOT_IDX_PLACEHOLDER" data-action="prepare_deposit" data-pay-token="${data.k}">
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="icon-md" data-pay-slot="SLOT_PLACEHOLDER" data-pay-slot-idx="SLOT_IDX_PLACEHOLDER" data-action="prepare_deposit" data-pay-token="${data.k}">
			<path d="M8 15A7 7 0 118 1 7 7 0 018 15ZM8 16A8 8 0 108 0 8 8 0 008 16ZM10.604 6.146A.5.5 0 0110.604 6.854L7.604 9.854A.5.5 0 016.896 9.854L5.396 8.354A.5.5 0 116.104 7.646L7.25 8.793 9.896 6.146A.5.5 0 0110.604 6.146Z" data-pay-slot="SLOT_PLACEHOLDER" data-pay-slot-idx="SLOT_IDX_PLACEHOLDER" data-action="prepare_deposit" data-pay-token="${data.k}"/>
		</svg>
	</label>
</div>`;
	...
	return [`<input type="radio" name="boost_currency" id="Boost__Currency__${i}" class="visually-hidden" autocomplete="off" data-pay-token="${data.k}">
...
const set_original_wallet_assets = async () => {
	...
	await Promise.all((await Promise.all(promises)).map(async (v) => {
