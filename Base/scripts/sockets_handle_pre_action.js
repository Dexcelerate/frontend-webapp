...
const PRE_ACTIONS = {
	...
	'reset-copy-settings': async element => {
		load_settings('ModalCopyPositionSettings', DATA.selected_slot && (DATA.selected_copy_wallet || DATA.copy_wallet || (DATA.selected_copy_wallet = DATA.ZERO)) && (DATA.copy_settings[DATA.selected_copy_slot] || (DATA.copy_settings[DATA.selected_copy_slot] = {})) && (DATA.copy_settings[DATA.selected_copy_slot][DATA.selected_copy_wallet || DATA.copy_wallet] = { ...DATA.default_copy_settings, buy_method_ids: [], sell_method_ids: [], kosher_strainer: [], targets_percents: [], targets_triggers: [] }));
	},
	...
	'deposit': async element => {
		...
		DATA.deposit_amount = Big(document.getElementById(`slot-${element.dataset.slotIdx}-deposit-range`).value).mul(DATA.ETHER).div(DATA.WPEG_PRICE).round();
	...
	'create-slot': async element => {
		...
		let chain_id = selected_chain();
		await sendChain(chain_id, DATA.CHAINS[chain_id].SUBSCRIPTION, 'slot', _hex(1), _hex(DATA.conf.affiliate)).then(() => {
	...
	'toggle_copy_wallet_state': async element => {
		if (!DATA.conf.connected) {
			setTimeout(() => {
				document.querySelectorAll(`label[data-action="toggle_copy_wallet_state"] input.slot-${DATA.selected_slot}, label[data-action="toggle_copy_wallet_state"] input.slot-wallet-${DATA.selected_slot}`).forEach(el => { el.checked = false });
			}, 0);
			on_connect(() => {
				_handleAction(element);
			});
		...
		if (DATA.selected_copy_wallet === DATA.ZERO) {
			...
			setTimeout(() => {
				document.querySelectorAll(`label[data-action="toggle_copy_wallet_state"] input.slot-${DATA.selected_slot}, label[data-action="toggle_copy_wallet_state"] input.slot-wallet-${DATA.selected_slot}`).forEach(el => { el.checked = !DATA.slot_toggle_is_spectator });
			}, 0);

			save_settings('slots', false, true);
		} else {
			DATA.slot_toggle_is_spectator =
				(DATA.copy_settings[DATA.selected_copy_slot][DATA.selected_copy_wallet].is_spectator = !DATA.copy_settings[DATA.selected_copy_slot][DATA.selected_copy_wallet].is_spectator);

			if (!DATA.slot_toggle_is_spectator && !DATA.slots[DATA.CHAIN][idx].is_active) {
				document.querySelectorAll(`label[data-action="toggle_copy_wallet_state"] input.slot-${DATA.selected_slot}`).forEach(el => { el.checked = true });
	...
	'load-wallet-settings': async element => {
		...
		if (!DATA.copy_settings[DATA.selected_copy_slot][DATA.selected_copy_wallet]) {
			DATA.copy_settings[DATA.selected_copy_slot][DATA.selected_copy_wallet] = { ...DATA.default_copy_settings, buy_method_ids: [], sell_method_ids: [], kosher_strainer: [], targets_percents: [], targets_triggers: [] }
		}
	...
	'claim': async element => {
		...
		if (DATA.conf.JBU) {
			...
			await sendChain(chain_id, DATA.CHAINS[chain_id].USERS_NFT, 'claim', _hex(DATA.conf.JBU)).catch(wallet_error((e, err) => {
				console.log('???', err);
	...
	'mint': async element => { /* JBU: */
		...
		if (!DATA.conf.JBU) {
			await sendChain(selected_chain(), DATA.SUBSCRIPTION, 'mint', _hex(DATA.conf.affiliate)).catch(e => { console.error(e); return false });
		}
	...
	'synagogize': async element => {
		...
		if (!(await sendChain(selected_chain(), DATA.SUBSCRIPTION, 'setPlan', _hex(elementify('synagogue-monthly-price').value), _hex(elementify('synagogue-max-minyan').value), true, nodes).catch(e => { console.error(e); return false }))) {
			return '-';
		}
	},
	...
	'sell': async element => {
		if (element.dataset.mainSell) {
			...
			DATA.wallet_token_sell_amount = await contract(DATA.selected_token).balanceOf(DATA.conf.wallet).catch(_ => 0);
