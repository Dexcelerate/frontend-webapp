...
var DATA = {
	...
	IP_API: 'https://ip.jewbot.trade/',
	IPFS_IMG_CDN: 'https://ipfs.io/ipfs/',
	DEX_IMG_CDN: 'https://jewbot.trade/img/vendor/exchanges/',
	TOKEN_IMG_CDN: 'https://img-proxy.jewbot.trade:3611/https://jewbot.trade/img/vendor/',
	TRUST_WALLET_CDN: 'https://img-proxy.jewbot.trade:3611/https://assets-cdn.trustwallet.com/blockchains/',
	view_uid: 0,
	view_user: {},
	view_rank: {},
	view_rank_history: [],
	...
	assets: [
		...
		{ name: 'Super Long Name That Should Overflow And Break All The Fucking Layout', favorited: true, balance: '$123456789.123456789', price: '$123456789.123456789', liquidity: '$123456789.123456789', volume: '$123456789.123456789', change: '123456789.123456789%' },
	]
};
...
const set_token = async (item, stop_reload) => {
	...
	if (!item && !(item = document.querySelector(`#ModalTokenOutput__Body .list .item[data-token-address="${DATA.token}"]`))) {
		await Promise.all([
			...
			(async () => {
				DATA.token_name = DATA.token && (await contract(DATA.token, undefined, undefined, true).name().catch(e => '')).replace('"', '&quot;').replace('<', '&lt;').replace('>', '&gt;').replace('\\', '') || 'Token';
			})(),
		]);
	...
	return (is_hidden) => {
		...
		document.querySelector('#Chart__Currency label img.dex-icon').setAttribute('onerror', `error_img(this, '${DATA.token_data.r && DATA.token_data.r.length && get_dex_image(DATA.ROUTER_NAMES[DATA.token_data.r[DATA.token_pair_idx].router]) || DATA.ERROR_IMG}')`);
...
DATA.slot = JSON.parse(store.get(`${DATA.CHAIN}_slot`) || 'null') || (~DATA.wallet_slots_pagination_idx && DATA.slots[DATA.CHAIN] && DATA.slots[DATA.CHAIN][DATA.wallet_slots_pagination_idx] && DATA.slots[DATA.CHAIN][DATA.wallet_slots_pagination_idx].address) || DATA.ZERO;
...
DATA.selected_sell_slot = DATA.selected_slot = ~DATA.wallet_slots_pagination_idx && DATA.slots[DATA.CHAIN] && DATA.slots[DATA.CHAIN][DATA.wallet_slots_pagination_idx] && DATA.slots[DATA.CHAIN][DATA.wallet_slots_pagination_idx].address || DATA.slot;
