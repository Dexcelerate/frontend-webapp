...
const update_balances = async () => {
    ...
    let user_balance, user_wpeg_balance, tmp;
    ...
    if (
        DATA.elements.main_sub_wallet_balance.dataset.balance !==
        (tmp = Big(DATA.conf.vault)
            .add(DATA.slots[DATA.CHAIN].reduce((a, b) => a.add(b.total_balance || 0), Big(0)))
            .toString())
    ) {
        DATA.elements.main_sub_wallet_balance.dataset.balance =
    ...
    document.querySelectorAll(`.slot-currencies-container`).forEach((el) => {
        if ((user_balance.gt(0) && !el.querySelector(`[data-pay-token="${DATA.IMAGINARY_PEG}"]`)) || (user_wpeg_balance.gt(0) && !el.querySelector(`[data-pay-token="${DATA.WPEG}"]`))) {
            setTimeout(async () => {
...
const update_price = async (i) => {
    ...
    if (typeof DATA.conf.nft_balance === undefined || (Number(DATA.conf.JBU) && i % 5 === 0)) {
        ...
        DATA.conf.nft_balance = Big(
            await contract(DATA.USERS_NFT, undefined, undefined, true)
                .getUserBalance(DATA.conf.JBU)
                .catch((_) => 0)
        ).div(DATA.ETHER);
...
const fetch_account_data = async function (last_operation) {
    ...
    document.querySelectorAll('[data-peg-text]').forEach((el) => {
        el.dataset.pegText = DATA.PEG;
    });
    document.querySelectorAll('img.peg').forEach((el) => {
        GetTokenImage(DATA.WPEG, undefined, el.id);
    });
...
const checkChainValue = async (chain_id, token, func, value, ...args) => {
    ...
    try {
        tx.gasLimit = _hex(
            Big(1.2)
                .mul(await provider[chain_id].estimateGas(tx))
                .round()
        );
    } catch (e) {
        return [e, null];
