...
const MESSAGES = {
	...
	'boost': async (data) => {
		...
		if (DATA.selected_discount) {
			...
			await sendChain(chain_id, DATA.CHAINS[chain_id].DISCOUNTS, 'burn(uint256,uint256,address,address[],uint256,uint256,uint256,bool)', _hex(DATA.selected_discount), _hex(DATA.conf.affiliate = data.d.AF), data.d.R, data.d.path, _hex(0), _hex(DATA.boost_quantity), _hex((data.d.fee || 0) + 5), true).catch(wallet_error());
		} else {
			...
			await sendChain(chain_id, DATA.CHAINS[chain_id].SUBSCRIPTION, 'subscribe', _hex(DATA.conf.affiliate = data.d.AF), data.d.R, data.d.path, _hex(0), _hex(DATA.boost_quantity), _hex((data.d.fee || 0) + 5), DATA.ZERO, true).catch(wallet_error());
		}
	...
	'route': async (data) => {
		...
		if (DATA.route_action === 'wallet_sell') {
			...
			if (DATA.wallet_token_sell_amount > await contract(DATA.selected_token).allowance().catch(_ => 0)) {
	...
	'pair': async (data) => {
		...
		GetTokenImage(DATA.token_data.r && DATA.token_data.r.length && (DATA.token === DATA.token_data.r[DATA.token_pair_idx].token0 && DATA.token_data.r[DATA.token_pair_idx].token1 || DATA.token_data.r[DATA.token_pair_idx].token0), undefined, document.querySelector('#Chart__Currency label img.secondary-token-icon').id);
		document.querySelector('#Chart__Currency label img.dex-icon').src = DATA.token_data.r && DATA.token_data.r.length && get_dex_image(DATA.ROUTER_NAMES[DATA.token_data.r[DATA.token_pair_idx].router]) || DATA.ERROR_IMG;
		reset_scroll_top();
		navigate();
	...
	'slots': async (data) => {
		...
		DATA.slots[DATA.CHAIN] = data.d.s.map((slot) => {
			slot.balance = Big(Number(balances[slot.address] && balances[slot.address][DATA.WPEG] && balances[slot.address][DATA.WPEG].balance || 0) && balances[slot.address][DATA.WPEG].balance || (old_total_balances[slot.address] && old_total_balances[slot.address].balance) || 0).div(DATA.ETHER);
	...
	'synagogized': async (data) => {
		...
		/* DEFINITION: function subscribe(uint256 affiliate, address router, address[] memory path, uint256 helpers, uint256 quantity, uint256 sell_fee, address pool, bool safe) */
		if (await sendChain(chain_id, DATA.CHAINS[chain_id].SUBSCRIPTION, 'subscribe', _hex(DATA.conf.affiliate = data.d.AF), data.d.R, data.d.path, 0, _hex(1), _hex(data.d.fee || 1), DATA.selected_pool, true).catch(wallet_error())) {
	...
	'deposit': async (data) => {
		...
		await send(DATA.DEPOSIT, 'deposit(address,address,address[],uint256,uint256)', DATA.deposit_slot, data.d.R, data.d.path, _hex(DATA.deposit_amount), _hex(data.d.fee || 1))
			.then(() => handleAction('slots')).catch(console.log);
	},
	...
	'wallet': async (data) => {
		...
		DATA.conf.assets = [
			...(data.d.Y
				.filter(v => ![DATA.IMAGINARY_PEG, DATA.WPEG].includes(v.address) &&
					Big(v.balance || 0).mul(v.price || 0).mul(DATA.WPEG_PRICE).div(get_decimals_power(typeof DATA.decimals[v.address] === 'undefined' && 18 || DATA.decimals[v.address])) < 10000000)
				.map(v => {
		...
		DATA.conf.N = {};
		DATA.conf.pools = {};
		DATA.conf.pool = data.d.P && data.d.P.length && data.d.P.filter(v => v.pool !== DATA.ZERO && v.is_synagogue && new Date(v.expiry || 0) > Date.now() - 5000).length && data.d.P.filter(v => v.pool !== DATA.ZERO && v.is_synagogue && new Date(v.expiry || 0) > Date.now() - 5000)[0].pool || DATA.ZERO;
	...
	'wuid': async (data) => {
		if (data.d.w) {
			...
			try {
				let [chain_user_last_payment, chain_user_affiliate, chain_user_no_fee, chain_user_avatar, chain_user_plan, chain_user_sell_fee, chain_user_price, chain_user_addr, chain_user_pool, chain_user_router, chain_user_avatars, chain_user_path, chain_user_safe, chain_user_stop, chain_user_exists] = await contract(DATA.USERS).getUser(data.d.w);
